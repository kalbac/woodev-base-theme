...
> violation. This theme is HYBRID CLASSIC (PHP templates + theme.json, see ADR-001): no FSE, no block
> templates, no Site Editor. Apply classic-theme + theme.json guidance; FSE/block-template sections of
> this skill are out of scope. Source: jorgerosal/wordpress-skills (vetted s1, 17.07.2026).
...
**Note:** This skill reviews PHP files (functions.php, classic templates), HTML block template files (templates/, parts/), JSON files (theme.json, style variations), and PHP pattern files (patterns/). PHP follows WordPress PHP Coding Standards (spaces in parentheses, Yoda conditions; NOTE project override: `[]` short array syntax, not `array()`). HTML templates use valid block markup. JSON follows theme.json v3 schema.
...
Common theme patterns organized by concern. PHP examples use WordPress PHP Coding Standards (spaces in parentheses, Yoda conditions; NOTE project override: `[]` short array syntax, not `array()`). HTML templates use valid block markup. JSON examples use valid theme.json v3 syntax.
