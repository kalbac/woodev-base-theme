...
- **Since M1-03 (s5) there is no `src/css/app.css`** — it was replaced by one generated entry per Basecoat style pack, `src/css/packs/<pack>.css` (gitignored). The load-bearing import order now lives in the generator template `packEntryCss()` in `scripts/lib/packs-lib.mjs`, and is the same rule as before: `@import 'basecoat-css/<pack>';` with **no** `layer()` wrapper, and `@import '../tokens.generated.css';` **after** it. Moving the tokens line above the pack silently discards every token — in all 8 bundles at once. `tests/js/build-pack-entries.test.mjs` pins that ordering ("imports the pack before the design tokens"); a build-time sentinel is to grep the built bundle and confirm the **last** `--font-sans` is our system stack, not Basecoat's `"Geist Sans"`.
