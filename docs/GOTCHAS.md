...
> 7 entries. Each gotcha is a separate file in `docs/gotchas/`.
...
| [wp-json-file-decode-warns-on-missing-file](gotchas/wp-json-file-decode-warns-on-missing-file.md) | PHP/WP core | It calls `wp_trigger_error()` before returning null — an absent manifest emits a PHP notice on every request. Guard with `is_file()`; returning null is no proof an API stayed quiet |
