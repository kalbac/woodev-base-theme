...
- **Dev mode has no integration/e2e coverage** — Codex P2 from s3, still open and still **unblocked**. The dev path is pinned only by unit tests with a mocked `wp_enqueue_script_module()`. Needs a second wp-env environment carrying `WOODEV_BASE_DEV`; the `.wp-env.<variant>.json` mechanism exists and is proven. Manual recipe: `docs/gotchas/wp-env-config-constants-persist.md`. M1-04 added a *known dev-mode limitation* that this coverage would have caught (below), which raises its value.
